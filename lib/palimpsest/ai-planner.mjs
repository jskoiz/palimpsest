...
The generated result must contain only the requested subject and its natural contact shadow on transparency. Any opaque rectangular background, matte, frame, visible square, or nontransparent color wash outside the subject fails.
  ...
  return {
    ...
    input: [{
      ...
      content: [
        {
          type: "input_text",
          text: `Requested change: ${requestedChange}\nFirst image: generated transparent layer. Second image: black protected pixels with a transparent editable area. Return whether every requested subject is complete with clear space on every side and whether all background outside the subject is transparent, with no opaque rectangle or color wash.`,
        },
      ...
      format: {
        ...
        schema: {
          ...
          properties: {
            ...
            backgroundClear: { type: "boolean" },
            reason: { type: "string" },
          },
          required: ["contained", "backgroundClear", "reason"],
...
export function extractContainmentReview(responseBody) {
  ...
  try {
    ...
    if (
      ...
      typeof parsed?.backgroundClear !== "boolean" ||
    ...
    return {
      ...
      backgroundClear: parsed.backgroundClear,
