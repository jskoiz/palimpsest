...
const REFERENCE_EDIT_REVIEW_INSTRUCTIONS = `Inspect a reference placement before it is accepted.
The first image is the generated current-canvas frame, the second is the original current-canvas frame before the edit, the third is the contributor's uploaded visual reference, and the fourth shows the exact placement area as transparency surrounded by protected black pixels.
Judge the requested subject only inside the transparent placement area. Protected black pixels are clipped by the app and are not part of the accepted layer.
...
Its position and relative scale must match the contributor's preview: centered in the transparent placement area, comfortably clear of every edge, and neither materially enlarged nor reduced to a tiny subject.
It must blend naturally into the current canvas with no visible rectangular patch, matte, frame, halo, color wash, or abrupt tonal seam.
Compare the generated frame with the original frame. Existing text, marks, and artwork inside the placement area must remain visually unchanged wherever the new subject or its natural contact shadow does not physically cover them.
...
Any material scale or position mismatch fails placement.
Any repainting, erasure, or smudging of uncovered prior artwork fails source preservation.
Ignore unrelated pre-existing content outside the placement area.`;
  ...
  return {
    ...
    input: [{
      ...
      content: [
        {
          type: "input_text",
          text: `Requested change: ${requestedChange}\nThe exact placement area in the first two images is x=${editableRegion.x}, y=${editableRegion.y}, width=${editableRegion.width}, height=${editableRegion.height}; its right edge is x=${right} and bottom edge is y=${bottom}. Return whether the subject is fully contained with clear space on every side, visually faithful to the uploaded reference, matched to the preview's placement and relative scale, naturally blended into the current canvas without a visible patch boundary, and whether uncovered prior artwork is preserved.`,
        },
        { type: "input_image", image_url: generatedImageUrl, detail: "high" },
...
/**
 * A reference edit either passes or returns one precise failure category.
 * The worker deliberately does not spend a second image-generation pass.
 * @param {{
 *   contained: boolean,
 *   faithful: boolean,
 ...
 * }} review
 */
export function referenceReviewOutcome(review) {
  if (
    review.contained &&
    review.faithful &&
  ...
  if (
    (!review.contained || !review.placementMatched) &&
    review.faithful &&
    review.blended &&
    review.sourcePreserved
  ) {
    return "reject-containment";
  }
