...
function extractResponseText(responseBody) {
  ...
  if (responseBody.status !== "completed") return null;
...
export function describeEditReviewResponse(responseBody) {
  ...
  for (const item of output) {
    ...
    if (typeof item.type === "string") outputTypes.push(item.type);
    const content = Array.isArray(item.content) ? item.content : [];
    for (const part of content) {
      if (!part || typeof part !== "object") continue;
      if (typeof part.type === "string") contentTypes.push(part.type);
  ...
  return {
    ...
    status: typeof responseBody.status === "string" ? responseBody.status : "missing",
    incompleteReason:
    ...
    outputTokens:
      typeof responseBody.usage?.output_tokens === "number"
        ...
        : null,
    reasoningTokens:
      typeof responseBody.usage?.output_tokens_details?.reasoning_tokens === "number"
...
The first image is the generated current-canvas frame, the second is the original current-canvas frame, the third is the contributor's positioned reference guide, and the fourth shows the editable area as transparency surrounded by protected black pixels. The reference guide may include its original source background.
The generated subject must be complete, uncropped, and surrounded by clear space on every side inside the editable area.
The result must remain unmistakably faithful to the primary subject in the reference guide, not its source background. Preserve the subject's distinctive silhouette, geometry, proportions, symbols, labels, fine construction details, and pale, reflective, transparent, translucent, frosted, glowing, or low-contrast material details instead of erasing, redesigning, or substituting them.
The primary subject's center and relative scale must match the positioned reference guide.
The source background behind the referenced subject must be omitted. The subject must be regenerated and naturally blended into the current canvas, with locally consistent lighting, texture, edge softness, and contact shadow and no visible rectangle, backdrop, matte, frame, halo, color wash, or abrupt tonal seam.
