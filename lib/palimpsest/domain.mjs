 ...
 * room to remain whole and the visible layer can fade into its source tile.
...
/**
 * Build the immutable display mask applied to a generated tile layer.
 * Filled patches use a feathered inner rectangle. Painted strokes keep a
 * narrow opaque core with a soft outer edge.
 * @param {{
...
export function createDisplayMaskSvg(mask) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024" viewBox="0 0 1024 1024"><defs><filter id="soft-edge" x="-40" y="-40" width="1104" height="1104" filterUnits="userSpaceOnUse"><feGaussianBlur stdDeviation="${MASK_FEATHER_RADIUS}"/></filter><clipPath id="edit-bounds"><rect x="${x}" y="${y}" width="${width}" height="${height}"/></clipPath></defs><g clip-path="url(#edit-bounds)"><g filter="url(#soft-edge)">${softContent}</g>${coreContent}</g></svg>`;
}

/**
 * OpenAI returns a complete replacement tile that already contains the source
 * outside the requested edit. Applying the input mask a second time can cut a
 * generated subject at the mask boundary, so only local demo overlays retain
 * a display mask.
 * @param {string} origin
 * @param {string | null | undefined} maskBlobId
 */
export function displayMaskForLayer(origin, maskBlobId) {
  return origin === "openai" ? null : maskBlobId ?? null;
}
 ...
 * Coordinates are tile-local; the tile itself is selected separately.
 * @param {any} input
 */
export function validateRegion(input) {
  ...
  const tileX = input.tile?.x;
  const tileY = input.tile?.y;
  const x = input.region?.x;
  const y = input.region?.y;
  const width = input.region?.width;
  const height = input.region?.height;

  if (![tileX, tileY, x, y, width, height].every(integer)) {
    throw new DomainError(
      "INVALID_REQUEST",
      "Tile and region coordinates must be whole numbers.",
    );
  }
  if (tileX < 0 || tileX > 1 || tileY < 0 || tileY > 1) {
    throw new DomainError("REGION_OUT_OF_BOUNDS", "Choose one of the four artwork tiles.");
  }
  ...
  if (x < 0 || y < 0 || x + width > TILE_SIZE || y + height > TILE_SIZE) {
    throw new DomainError(
      "REGION_OUT_OF_BOUNDS",
      "The editable patch must stay inside one tile.",
    );
  }
  ...
  const region =
    row.regionX == null
      ...
      : {
          ...
          tile: { x: Number(row.tileX), y: Number(row.tileY) },
        };
...
/**
 * Resolve immutable base tiles plus ordered patch layers. A revert resets the
 * visible layer stack to the target revision, then continues forward.
 * @param {any[]} revisions
 * @param {any[]} baseTiles
 * @param {any[]} patches
 */
export function resolveTileLayers(revisions, baseTiles, patches) {
  const patchMap = new Map();
  for (const patch of patches) {
    const group = patchMap.get(patch.revisionId) ?? [];
    group.push(patch);
    patchMap.set(patch.revisionId, group);
  }

  const cloneTiles = (/** @type {Map<string, any>} */ source) => {
    const result = new Map();
    for (const [key, value] of source) {
      result.set(key, { ...value, layers: [...value.layers] });
    }
    return result;
  };
  ...
  for (const tile of baseTiles) {
    initial.set(`${tile.tileX}:${tile.tileY}`, {
      x: Number(tile.tileX),
      y: Number(tile.tileY),
      base: tile,
      layers: [],
  ...
  for (const revision of [...revisions].sort(
    ...
    if (revision.origin === "revert" && revision.revertTargetRevisionId) {
      ...
      current = cloneTiles(target);
    } else {
      current = cloneTiles(current);
    }

    for (const patch of patchMap.get(revision.id) ?? []) {
      const key = `${patch.tileX}:${patch.tileY}`;
      const tile = current.get(key);
      if (!tile) {
        throw new DomainError("INTERNAL_ERROR", "A revision patch references an unknown tile.");
      }
      tile.layers.push(patch);
    }
    snapshots.set(revision.id, cloneTiles(current));
  }

  return [...current.values()].sort((a, b) => a.y - b.y || a.x - b.x);
}
