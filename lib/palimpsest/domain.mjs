...
export const REFERENCE_EDIT_MIN_EDGE = 320;
export const REFERENCE_TARGET_FILL = 0.35;
...
/**
 * Resolve the exact centered footprint shown by the reference preview.
 * The outer edit region remains available as generation context, while only
 * this inner region may become visible in the accepted reference layer.
 * @param {{x: number, y: number, width: number, height: number}} region
 */
export function referencePlacementRegion(region) {
  const width = Math.max(1, Math.round(region.width * REFERENCE_TARGET_FILL));
  const height = Math.max(1, Math.round(region.height * REFERENCE_TARGET_FILL));
  return {
 ...
 */
export function buildOpenAiEditPrompt(
  change,
  hasReference = false,
  containmentRetry = false,
) {
  return [
    "Image 1 is a high-resolution working crop of the current canvas. Edit it only inside the transparent part of the mask.",
    hasReference
      ? "The transparent mask area is the exact placement footprint shown to the contributor. Image 2 is the contributor's full-resolution visual reference and the identity and detail source. Place its primary subject centered within that footprint at the same relative scale shown by the reference preview; do not enlarge, crop, reposition, redesign, simplify, substitute, or make it materially smaller."
      : "",
    hasReference
      ? "Remove or replace Image 2's original background while preserving the visible primary subject and blending it into the surrounding canvas in Image 1."
      : "",
    hasReference
      ? "Preserve the primary subject's identifying silhouette, geometry, proportions, perspective, materials, control count and arrangement, symbols, labels, and fine construction details. Treat those details as fixed visual evidence, not suggestions to improvise."
      : "",
    hasReference
      ? "Blend the faithful subject into the current canvas surroundings by matching local perspective, scale, lighting, texture, edge softness, and contact shadow. Do not copy the reference image's rectangular background or leave a visible rectangle, matte, halo, wash, frame, or tonal seam."
    ...
    hasReference
      ? "Preserve every visible component of the primary referenced subject and do not invent hidden structure or replacement controls. Preserve every existing Image 1 pixel that the new subject or its natural contact shadow does not physically cover, including existing text and marks. Reference backgrounds, display stands, or secondary props may be omitted unless the requested change explicitly includes them."
    ...
    hasReference
      ? "Keep the complete subject centered inside the transparent placement footprint with clear space on all four sides. The surrounding opaque mask is protected prior artwork, not extra room for the subject."
      : "If a new subject approaches the editable edge, scale it down instead of cropping it.",
    hasReference && containmentRetry
      ? "This is the single placement retry. Match the reference preview's scale and center exactly inside the transparent placement footprint. Keep the complete subject clear of every edge without shrinking it below half of the footprint's width or height."
      : "",
...
/**
 * Reference-guided edits need enough canonical canvas space for subject detail,
 * contextual blending, and visible breathing room.
 * @param {{width: number, height: number}} region
 */
export function assertReferenceEditRegion(region) {
  if (
    region.width < REFERENCE_EDIT_MIN_EDGE ||
    region.height < REFERENCE_EDIT_MIN_EDGE
  ) {
    throw new DomainError(
      "REFERENCE_REGION_TOO_SMALL",
      `Reference images need an edit patch at least ${REFERENCE_EDIT_MIN_EDGE} by ${REFERENCE_EDIT_MIN_EDGE} pixels.`,
    );
  }
  return true;
}
