...
export const REFERENCE_TARGET_FILL = 0.6;
export const GENERATION_SEAM_GUARD = 48;
...
/**
 * Preserve a hard ring of source pixels around filled generation masks. This
 * gives the model an exact local color reference at every boundary without
 * feathering the immutable display mask or reducing the accepted patch.
 * @param {{width: number, height: number}} region
 */
export function generationMaskInset(region) {
  const shortestEdge = Math.min(region.width, region.height);
  const minimumEditableEdge = Math.max(
    MIN_EDIT_EDGE,
    Math.ceil(shortestEdge * 0.75),
  );
  const availableInset = Math.floor((shortestEdge - minimumEditableEdge) / 2);
  return Math.max(0, Math.min(GENERATION_SEAM_GUARD, availableInset));
}
...
export function buildOpenAiEditPrompt(change, hasReference = false) {
  return [
    ...
    "Treat the protected source pixels immediately around the editable area as the exact seam reference. Match their local color, gradient, illumination, and texture at every boundary; do not leave a visible rectangle, corner, halo, or tonal shift.",
