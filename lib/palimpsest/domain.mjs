...
export function buildOpenAiEditPrompt(change, hasReference = false) {
  return [
    "Edit the supplied current canvas image only inside the transparent part of the mask.",
    hasReference
      ? "The supplied current-canvas image already contains the contributor's reference at the intended position and size. Treat those visible pixels as the identity, detail, and composition source. Keep the primary subject within that exact footprint; do not enlarge, crop, reposition, redesign, simplify, or substitute it."
      : "",
    hasReference
      ? "Remove or replace the reference image's original background while preserving the visible primary subject and blending it into the surrounding canvas."
      : "",
    hasReference
