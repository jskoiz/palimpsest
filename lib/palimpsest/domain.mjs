...
/**
 * Uploaded placement layers already carry their exact compositing boundary in
 * their alpha channel. This mask is only a hard outer clip that prevents any
 * pixel from escaping the server-authoritative edit region.
 * @param {{x: number, y: number, width: number, height: number}} region
 */
export function createPlacementDisplayMaskSvg(region) {
  const { x, y, width, height } = region;
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${GENERATION_FRAME_SIZE}" height="${GENERATION_FRAME_SIZE}" viewBox="0 0 ${GENERATION_FRAME_SIZE} ${GENERATION_FRAME_SIZE}"><rect x="${x}" y="${y}" width="${width}" height="${height}" fill="white"/></svg>`;
}
