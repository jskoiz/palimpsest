...
export const REFERENCE_PLACEMENT_MIN_EDGE = MIN_EDIT_EDGE;
...
/**
 * Resolve integer dimensions for an aspect-locked placement from its long edge.
 *
 * @param {number} longEdge
 * @param {number} aspectRatio width divided by height
 */
function referencePlacementSize(longEdge, aspectRatio) {
...
/**
 * Find the largest integer long edge that fits an aspect-locked placement
 * inside the supplied width and height.
 *
 * @param {number} maximumWidth
 * @param {number} maximumHeight
 * @param {number} aspectRatio width divided by height
 */
function maximumReferenceLongEdge(maximumWidth, maximumHeight, aspectRatio) {
  if (aspectRatio >= 1) {
    return Math.max(
      1,
      Math.floor(Math.min(maximumWidth, maximumHeight * aspectRatio)),
    );
  }
  return Math.max(
    1,
    Math.floor(Math.min(maximumHeight, maximumWidth / aspectRatio)),
  );
}

/**
 * Return the minimum long edge that gives the placement a server-valid short
 * edge.
 *
 * @param {number} aspectRatio width divided by height
 */
function minimumReferenceLongEdge(aspectRatio) {
  return Math.ceil(
...
/**
 * Create the initial exact placement rectangle for an uploaded reference.
 * The rectangle is centered over the user's current patch, preserves the
 * uploaded image's aspect ratio, and stays within the 2048px artwork. It fills
 * as much of the user's chosen patch as the uploaded aspect ratio permits,
 * grows only when a feasible server-valid short edge requires it, and never
 * exceeds 512px. The visible preview is therefore the exact accepted footprint
 * rather than a smaller subject hidden inside a larger generation area.
 *
 * @param {Rectangle} current
 * @param {number} aspectRatio width divided by height
 * @returns {Rectangle}
 */
export function initialReferencePlacementRegion(current, aspectRatio) {
  const normalizedAspect = normalizedAspectRatio(aspectRatio);
  const fittedLongEdge = maximumReferenceLongEdge(
    Math.min(EDIT_REGION_MAX_EDGE, current.width),
    Math.min(EDIT_REGION_MAX_EDGE, current.height),
    normalizedAspect,
  );
  const maximumLongEdge = maximumReferenceLongEdge(
    EDIT_REGION_MAX_EDGE,
    EDIT_REGION_MAX_EDGE,
    normalizedAspect,
  );
  const minimumLongEdge = minimumReferenceLongEdge(normalizedAspect);
  const longEdge = clamp(
    Math.max(
      fittedLongEdge,
      minimumLongEdge <= maximumLongEdge ? minimumLongEdge : 1,
    ),
    1,
    maximumLongEdge,
  );
  const size = referencePlacementSize(longEdge, normalizedAspect);
  const centered = {
    x: Math.round(current.x + current.width / 2 - size.width / 2),
    y: Math.round(current.y + current.height / 2 - size.height / 2),
    ...size,
  };
...
/**
 * Resize an exact reference placement while keeping its top-left corner fixed.
 * Both desired dimensions influence the scale. The result preserves the
 * supplied aspect ratio to integer-pixel precision, keeps the short edge at
 * least the server minimum whenever the remaining canvas can support it, and
 * never exceeds either the 512px edit limit or the available artwork bounds.
 *
 * @param {Rectangle} current
 * @param {number} desiredWidth
 * @param {number} desiredHeight
 ...
 * @returns {Rectangle}
 */
export function resizeReferencePlacementRegion(
  current,
  desiredWidth,
  desiredHeight,
  aspectRatio,
  ...
  const maximumWidth = Math.max(
    1,
    Math.min(EDIT_REGION_MAX_EDGE, ARTWORK_SIZE - current.x),
  );
  const maximumHeight = Math.max(
    1,
    Math.min(EDIT_REGION_MAX_EDGE, ARTWORK_SIZE - current.y),
  );
  const maximumLongEdge = maximumReferenceLongEdge(
    maximumWidth,
    maximumHeight,
  ...
  const safeDesiredWidth = Number.isFinite(desiredWidth)
    ? Math.max(1, desiredWidth)
    : current.width;
  const safeDesiredHeight = Number.isFinite(desiredHeight)
    ? Math.max(1, desiredHeight)
    : current.height;
  const projectedLongEdge =
    normalizedAspect >= 1
      ? (safeDesiredWidth + safeDesiredHeight / normalizedAspect) /
        (1 + 1 / normalizedAspect ** 2)
      : (safeDesiredWidth * normalizedAspect + safeDesiredHeight) /
        (normalizedAspect ** 2 + 1);
  const minimumLongEdge = minimumReferenceLongEdge(normalizedAspect);
  const longEdge = clamp(
    Math.round(projectedLongEdge),
    minimumLongEdge <= maximumLongEdge ? minimumLongEdge : maximumLongEdge,
    maximumLongEdge,
  );
  const size = referencePlacementSize(longEdge, normalizedAspect);
  return {
