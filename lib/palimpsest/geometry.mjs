 ...
 * when possible and clamps at artwork edges.
 *
...
export function generationFrameForRegion(region) {
  ...
  const frameMaximum = ARTWORK_SIZE - edge;
  return {
    x: clamp(
      Math.round(region.x + region.width / 2 - edge / 2),
      0,
      frameMaximum,
    ),
    y: clamp(
      Math.round(region.y + region.height / 2 - edge / 2),
      0,
      frameMaximum,
    ),
    width: edge,
