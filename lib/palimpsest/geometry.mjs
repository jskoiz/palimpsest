 ...
 */
function sweepAxis(start, delta, minimum, maximum) {
  if (delta === 0) {
...
/**
 * Find the first positive-area collision along a patch's motion segment. Each
 * reservation is expanded by the patch dimensions, reducing swept rectangle
 * collision to a moving point. Edge and corner contact remain valid.
 *
 * @param {Rectangle} start
 * @param {Rectangle} end
 * @param {Rectangle[]} blockers
 */
function firstSweepCollision(start, end, blockers) {
  const deltaX = end.x - start.x;
  const deltaY = end.y - start.y;
  /** @type {{time: number, axis: "x" | "y" | "corner"} | null} */
  ...
  for (const blocker of blockers) {
    const xSweep = sweepAxis(
      start.x,
      deltaX,
      blocker.x - start.width,
      blocker.x + blocker.width,
    );
    const ySweep = sweepAxis(
      start.y,
      deltaY,
      blocker.y - start.height,
      blocker.y + blocker.height,
    );
    if (!xSweep || !ySweep) continue;

    const entry = Math.max(xSweep.entry, ySweep.entry);
    const exit = Math.min(xSweep.exit, ySweep.exit);
    if (entry >= exit || exit <= 0 || entry < 0 || entry > 1) continue;

    const axis =
      xSweep.entry > ySweep.entry
        ? "x"
        : ySweep.entry > xSweep.entry
          ? "y"
          : "corner";
    if (!firstCollision || entry < firstCollision.time) {
      firstCollision = { time: entry, axis };
    }
  }
 ...
 * @param {number} time
 */
function regionAtSweepTime(start, end, time) {
  return positionEditRegion(
    start,
...
/**
 * Find an open placement for a patch that has become covered by a newly
 * arrived reservation. This is intentionally only a recovery path; ordinary
 * dragging uses swept collision and cannot teleport through a lock.
 *
 * @param {Rectangle} current
 * @param {Rectangle} desired
 * @param {Rectangle[]} blockers
 */
function nearestOpenRegion(current, desired, blockers) {
  const maximumX = ARTWORK_SIZE - current.width;
  const maximumY = ARTWORK_SIZE - current.height;
  const xPositions = new Set([desired.x, current.x, 0, maximumX]);
  const yPositions = new Set([desired.y, current.y, 0, maximumY]);
  for (const blocker of blockers) {
    xPositions.add(positionAxis(blocker.x - current.width, current.width));
    xPositions.add(positionAxis(blocker.x + blocker.width, current.width));
    yPositions.add(positionAxis(blocker.y - current.height, current.height));
    yPositions.add(positionAxis(blocker.y + blocker.height, current.height));
  }
  ...
  for (const x of xPositions) {
    for (const y of yPositions) {
      const candidate = positionEditRegion(current, x, y);
      const key = `${candidate.x}:${candidate.y}`;
      if (seen.has(key)) continue;
      seen.add(key);
      if (!blockers.some((region) => regionsOverlap(candidate, region))) {
  ...
  candidates.sort((a, b) => {
    ...
    const aTravel = (a.x - current.x) ** 2 + (a.y - current.y) ** 2;
    const bTravel = (b.x - current.x) ** 2 + (b.y - current.y) ** 2;
...
/**
 * Move a patch toward a desired point without occupying or tunneling through
 * any live reserved region. Edge and corner contact remain valid; blocked
 * motion stops at the first lock and continues along its open axis.
 *
 * @param {Rectangle} current
 * @param {number} desiredGlobalX
 * @param {number} desiredGlobalY
 * @param {Rectangle[]} blockedRegions
 * @returns {Rectangle}
 */
export function positionEditRegionAvoidingRegions(
  current,
  desiredGlobalX,
  desiredGlobalY,
  blockedRegions,
) {
  const desired = positionEditRegion(current, desiredGlobalX, desiredGlobalY);
  const blockers = blockedRegions.filter(
    (region) => region.width > 0 && region.height > 0,
  );
  if (blockers.length === 0) return desired;

  if (blockers.some((region) => regionsOverlap(current, region))) {
    return nearestOpenRegion(current, desired, blockers);
  }

  const collision = firstSweepCollision(current, desired, blockers);
  if (!collision) return desired;

  const contact = regionAtSweepTime(current, desired, collision.time);
  if (collision.axis === "corner") return contact;

  const slideTarget = positionEditRegion(
    contact,
    collision.axis === "x" ? contact.x : desired.x,
    collision.axis === "y" ? contact.y : desired.y,
  );
  const slideCollision = firstSweepCollision(contact, slideTarget, blockers);
  const placed = slideCollision
    ? regionAtSweepTime(contact, slideTarget, slideCollision.time)
    : slideTarget;
  return blockers.some((region) => regionsOverlap(placed, region))
    ? contact
    : placed;
}
...
/**
 * Resize without entering a live reservation. If the full diagonal resize is
 * blocked, preserve whichever open axis can still move.
 *
 * @param {Rectangle} current
 * @param {number} desiredWidth
 * @param {number} desiredHeight
 ...
 * @returns {Rectangle}
 */
export function resizeEditRegionAvoidingRegions(
  current,
  desiredWidth,
  desiredHeight,
  blockedRegions,
) {
  const candidate = resizeEditRegion(current, desiredWidth, desiredHeight);
  const blockers = blockedRegions.filter(
    (region) => region.width > 0 && region.height > 0,
  );
  if (!blockers.some((region) => regionsOverlap(candidate, region))) {
  ...
  const widthOnly = { ...candidate, height: current.height };
  if (!blockers.some((region) => regionsOverlap(widthOnly, region))) {
    return widthOnly;
  }

  const heightOnly = { ...candidate, width: current.width };
  if (!blockers.some((region) => regionsOverlap(heightOnly, region))) {
    return heightOnly;
  }
