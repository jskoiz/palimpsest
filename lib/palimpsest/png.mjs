...
export const PLACEMENT_PNG_WIDTH = 1024;
export const PLACEMENT_PNG_HEIGHT = 1024;
export const MAX_PLACEMENT_PNG_BYTES = 6 * 1024 * 1024;

const PNG_SIGNATURE = [137, 80, 78, 71, 13, 10, 26, 10];
const RGBA_BYTES_PER_PIXEL = 4;
const MAX_PNG_CHUNKS = 4096;
const EXPECTED_SCANLINE_BYTES =
  1 + PLACEMENT_PNG_WIDTH * RGBA_BYTES_PER_PIXEL;
const EXPECTED_INFLATED_BYTES =
  PLACEMENT_PNG_HEIGHT * EXPECTED_SCANLINE_BYTES;
 ...
 * @returns {never}
 */
function invalidPlacementPng(message) {
...
function chunkType(bytes, offset) {
  ...
  if (
    ...
    invalidPlacementPng("The placed image contains an invalid PNG chunk.");
  }
...
async function inflateIdat(chunks, byteLength) {
  if (
    byteLength < 1 ||
    byteLength > MAX_PLACEMENT_PNG_BYTES
  ) {
    invalidPlacementPng("The placed image has invalid compressed pixel data.");
  }
  ...
  try {
    ...
    while (true) {
      ...
      if (
        ...
        invalidPlacementPng("The placed image expands beyond its declared frame.");
      }
      inflated.set(value, inflatedOffset);
      inflatedOffset += value.byteLength;
    }
    if (inflatedOffset !== inflated.byteLength) {
      invalidPlacementPng("The placed image has incomplete pixel data.");
    }
    return inflated;
  } catch (error) {
...
function validateScanlines(inflated) {
  const rowBytes = PLACEMENT_PNG_WIDTH * RGBA_BYTES_PER_PIXEL;
  ...
  for (let row = 0; row < PLACEMENT_PNG_HEIGHT; row += 1) {
    ...
    if (filter > 4) {
      invalidPlacementPng("The placed image uses an invalid PNG scanline filter.");
    }
  ...
  if (!hasTransparency || !hasVisiblePixel) {
    invalidPlacementPng(
      "The placed image must contain both visible and transparent pixels.",
    );
  }
}

/**
 * Parse and fully validate the one accepted placement-image contract.
 ...
 */
export async function validatePlacementPng(bytes) {
  if (
    !(bytes instanceof Uint8Array) ||
    bytes.byteLength < 45 ||
    bytes.byteLength > MAX_PLACEMENT_PNG_BYTES ||
    PNG_SIGNATURE.some((value, index) => bytes[index] !== value)
  ) {
    invalidPlacementPng("The placed image must be a valid PNG file.");
  }
  ...
  while (offset < bytes.byteLength) {
    ...
    if (chunkCount > MAX_PNG_CHUNKS || seenEnd || offset + 12 > bytes.byteLength) {
      invalidPlacementPng("The placed image has an invalid PNG chunk layout.");
    }
    ...
    if (
      dataLength > MAX_PLACEMENT_PNG_BYTES ||
      nextOffset > bytes.byteLength
    ) {
      invalidPlacementPng("The placed image has a truncated PNG chunk.");
    }
    if (
      crc32(bytes, typeOffset, crcOffset) !== readUint32(bytes, crcOffset)
    ) {
      invalidPlacementPng("The placed image failed PNG integrity validation.");
    }
    if (chunkCount === 1 && type !== "IHDR") {
      invalidPlacementPng("The placed image must begin with a PNG header.");
    }

    if (type === "IHDR") {
      if (seenHeader || dataLength !== 13 || chunkCount !== 1) {
        invalidPlacementPng("The placed image has an invalid PNG header.");
      }
      ...
      if (
        width !== PLACEMENT_PNG_WIDTH ||
        height !== PLACEMENT_PNG_HEIGHT ||
        bitDepth !== 8 ||
        colorType !== 6 ||
        compression !== 0 ||
        filter !== 0 ||
        ...
        invalidPlacementPng(
          "The placed image must be a non-interlaced 1024 by 1024 8-bit RGBA PNG.",
        );
      }
      seenHeader = true;
    } else if (type === "PLTE") {
      if (
        ...
        invalidPlacementPng("The placed image has an invalid PNG palette.");
      }
      seenPalette = true;
    } else if (type === "IDAT") {
      if (!seenHeader || imageDataEnded) {
        invalidPlacementPng("The placed image has non-contiguous PNG pixel data.");
      }
      seenImageData = true;
      compressedBytes += dataLength;
      if (compressedBytes > MAX_PLACEMENT_PNG_BYTES) {
        invalidPlacementPng("The placed image has too much compressed pixel data.");
      }
      imageDataChunks.push(bytes.subarray(dataOffset, crcOffset));
    } else if (type === "IEND") {
      if (!seenImageData || seenEnd || dataLength !== 0) {
        invalidPlacementPng("The placed image has an invalid PNG ending.");
      }
      seenEnd = true;
      if (nextOffset !== bytes.byteLength) {
        invalidPlacementPng("The placed image contains data after its PNG ending.");
      }
    } else {
      if ((bytes[typeOffset] & 0x20) === 0) {
        invalidPlacementPng("The placed image contains an unsupported PNG chunk.");
      }
  ...
  if (!seenHeader || !seenImageData || !seenEnd || offset !== bytes.byteLength) {
    invalidPlacementPng("The placed image is missing required PNG data.");
  }
